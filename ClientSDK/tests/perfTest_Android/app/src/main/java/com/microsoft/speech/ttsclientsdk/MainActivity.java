...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        m_syn.setReceiveWave(new ITTSCallback () {
            public int ReceiveWave(long responceHandle, final byte[] data, int size) {
        ...
        //init speech handle
        handle = m_syn.MSTTS_CreateSpeechSynthesizerHandler(handle, "Your API key");
        ...
        //set output
        if(result == 0){
            result = m_syn.MSTTS_SetOutput(handle, m_syn);
        }
